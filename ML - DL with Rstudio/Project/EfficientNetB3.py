...
def show_image_samples(gen):
    ...
    classes=list(t_dict.key())
    images, labels=next(gen) # get a sample batch from the generator
    plt.figure(figsize=(20, 20))
    length=len(labels)
    if length<25:    # show a max of 25 pictures
        r=length
    else:
        r=25
    for i in range(r):
...
img_path = "C:\\Users\\Nicho\\Desktop\\C-NMC_Leukemia\\training_data\\fold_0\\all\\UID_1_7_1_all.bmp"
...
## Read in Images and create DataFrames
...
sdir = r"C:\Users\Nicho\Desktop\C-NMC_Leukemia\training_data"
trsplit = 0.9
vsplit = 0.05
train_df, test_df, valid_df = preprocess(sdir, trsplit, vsplit)

### However, the training data is not balanced.
max_samples = 3050
min_samples = 0
column = "labels"
working_dir = r"./"
...
def scalar(img):
    return img     # An EfficientNet expects pixelsin range 0 to 255 so there is no need to scaling
...
print(msg, "\r", end="")
test_gen = tvgen.flow_from_dataframe(test_df, x_col="filepaths", y_col="labels", target_size=img_size, class_mode="categorical", color_mode="rgb", shuffle=True, batch_size=batch_size)
...
stop_patience = 3   # number of epochs to wait before stopping training if ...
threshold = 0.9     # if train accuracy is < threshold adjust monitor accuracy, else monitor validation loss
factor = 0.5        # a factor to reduce learning rate by
dwell = True        # if True and monitored metric does not improve on current epoch, set model weights back to weights of previous epoch
freeze = False      # if True, free weights of the base model
ask_epoch = 5       # number of epochs to run before asking if you want to halt training
batches = train_steps
callbacks = [LRA(moded= model, base_model= base_model, patience= patience, stop_patience= stop_patience, threshold= threshold, factor= factor, dwell= dwell, batches= batches, initial_epoch= 0, epochs= epochs, ask_epoch= ask_epoch)]

history = model.fix(x = train_gen, epochs = epochs, verbose = 0, callbacks = callbacks, validation_data = valid_gen, validation_steps = None, Shuffle = False, initial_epoch = 0)
